...
file_name = "protocol_params_reduced"
if_logtransform = True
model_name_list = ["LCEN2"]

if if_logtransform:
    with open("SPA_results_agnostic/log_" + file_name + ".pkl", 'rb') as file:
        history = pickle.load(file)
else:
    with open("SPA_results_agnostic/" + file_name + ".pkl", 'rb') as file:
        history = pickle.load(file)
...
for model_name in model_name_list:
    for i in list(history[model_name].keys()):
        ...
        elif model_name in {'EN', 'RR', 'PLS', 'SPLS'}:
