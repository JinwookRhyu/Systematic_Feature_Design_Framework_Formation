...
file_name = "log_B_Q_V_features_univariate_4.5"
model_name_list = ["SVR"]

with open("SPA_results_autoML/" + file_name + ".pkl", 'rb') as file:
...
Data_original = read_excel("Features_tsfresh_autoML/" + file_name + ".xlsx", header = None).values
            ...
            if model_name == "LCEN":
                ...
                for k in range(len(history[model_name][i]['model_params'])):
                    ...
                    param_value_new = np.append(param_value_new,
                                                history[model_name][i]['model_params'][k][1] / y.std() * X[:,
                                                                                                         col_ind].std(
                                                    axis=0))
