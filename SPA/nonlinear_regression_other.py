...
def RF_fitting(X, y, X_test, y_test, n_estimators = 100, max_depth = 10, min_samples_leaf = 0.1, max_features = 1.0,random_state=0):
    ...
    RF = RandomForestRegressor(n_estimators=n_estimators, max_depth=max_depth,random_state= random_state,
                                    max_features = max_features, min_samples_leaf = min_samples_leaf, n_jobs=-1)
    RF.fit(X, y.flatten())
...
def XGB_fitting(X, y, X_test, y_test, max_depth, reg_lambda, gamma, n_estimators, random_state=0):
    '''XGBoost https://scikit-learn.org/stable/modules/generated/sklearn.ensemble.RandomForestRegressor.html#sklearn.ensemble.RandomForestRegressor.decision_path
    Input:
    X: independent variables of size N x m
    y: dependent variable of size N x 1
    X_test: independent variables of size N_test x m
    ...
    # build model
    XGB = xgb.XGBRegressor(n_estimators=n_estimators, max_depth=max_depth, random_state=random_state,
                               reg_lambda = reg_lambda, gamma = gamma, n_jobs=-1)
    XGB.fit(X, y.flatten())

    # predict
    yhat_train = XGB.predict(X).reshape((-1, 1))
    yhat_test = XGB.predict(X_test).reshape((-1, 1))

    # get error
    mse_train = mean_squared_error(y, yhat_train)
    mse_test = mean_squared_error(y_test, yhat_test)

    return (XGB, mse_train, mse_test, yhat_train, yhat_test)
