...
def main_SPA(data_name, main_data, save_name, interrogation_plot_name=None, test_data = None, interpretable = False, continuity = False, group_name = None, spectral_data = False,
    ...
    # Non-dynamic models
    if any(temp_model in model_name for temp_model in {'ALVEN', 'LCEN', 'SVR', 'RF', 'RR', 'PLS', 'EN', 'PLS', 'SPLS', 'XGB'}) and 'OLS' not in model_name: # TODO: how do we compare OLS with the other models if OLS doesn't have validation scores?
        ...
        if not nested_cv:
            ...
            for index, model_index in enumerate(model_name):
                if model_index in {'ALVEN', 'LCEN', 'SVR', 'RF', 'RR', 'PLS', 'EN', 'PLS', 'SPLS',
                                   'XGB'}:  # There may be dynamic models if the user passed model_name manually
                    try:
        ...
        # Nested CV
            ...
            if group_name is None:
                ...
                for index_out in range(num_outer):
                    ...
                    for index, model_index in enumerate(model_name):
                        if model_index in {'ALVEN', 'LCEN', 'SVR', 'RF', 'RR', 'PLS', 'EN', 'PLS', 'SPLS', 'XGB'}: # There may be dynamic models if the user passed model_name manually
                            try:
                                print(f'Running model {model_index}', end='\r')
